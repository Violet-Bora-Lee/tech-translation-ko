...
- **옮긴이 주**: 번역 독자에게만 필요한 배경은 "(ECMAScript - 옮긴이)" 형식으로 삽입.
  ...
  용어 첫 등장의 정의 복원에도 쓴다. "문서가 언로드될 때(탭을 닫거나 다른 페이지로
  이동해서 현재 문서가 사라질 때 - 옮긴이)".
- **API 파라미터는 생략 시 동작까지**: "옵션 문자열" → "생략 가능한 문자열인 `type`",
  "기본값은 원본과 같습니다" → "생략하면 새 Blob은 원본 Blob의 `type`을 그대로
  물려받습니다". optional·default를 '무엇을 안 했을 때 무슨 일이 일어나는가'로 푼다.
...
개념 전달과 무관한 문화 레퍼런스는 한국 독자에게 친숙한 것으로 바꿀 수 있다.
현지화 수위는 프로젝트 정책이다. 튜토리얼·블로그처럼 어조가 자산인 글에서 통하는
기법이고, 공식 문서·API 레퍼런스는 원문 유지가 기본이다.
...
- **피동→능동 전환 시 주체 반전 주의**: "이를 인수로 `callback`이 호출됩니다" →
  "완성된 `Blob`을 인수로 넘겨 `callback`을 호출합니다"는 좋은 전환이지만, 관형절로
  압축하다 "인수로 넘기는 `callback`"처럼 행위 주체가 뒤집히면 오역이 된다. 전환 후
  '누가 누구에게'를 다시 확인한다.
- **가능문과 권고문 구분**: 문맥이 '문제 제시 → 해법' 구조면 can의 직역
  "변환할 수 있습니다"보다 "변환하는 편이 좋습니다"로 권고를 드러낸다. 단순히 선택지를
  나열하는 맥락이면 가능문을 유지한다.
