...
- 프로젝트 고유 링크·앵커 문법(예: javascript.info의 `<info:anchor-slug>`)은
  식별자다. 슬러그를 번역하거나 수정하지 않는다(린터도 이 구간은 용어 검사에서 제외한다).
- 독립 문서 번역(구조 보존이 계약이 아닌 경우)이라면 줄 수 규칙은 완화해도 되지만
  헤딩·코드블록·링크 구조는 여전히 보존한다. 이 스킬에선 독립 문서가 기본값이다.
...
- **에러의 경계선**: 언어·플랫폼이 **생성하는** 에러 출력은 원문을 유지한다
  ...
  치환 문구는 프로젝트 하우스 스타일로 정해 문서 전체에 일관 적용한다. 본문에서 실제
  에러 출력을 언급할 땐 '정의되지 않았다(not defined)'처럼 한-영 병기.
...
## Special blocks · 어드모니션·인용 블록

문서 프레임워크마다 콜아웃(admonition) 문법이 다르다(javascript.info의 ` ```smart `,
Docusaurus의 `:::note`, MkDocs의 `!!! note` 등). 문법 토큰은 유지하고 자연어만 번역한다.
