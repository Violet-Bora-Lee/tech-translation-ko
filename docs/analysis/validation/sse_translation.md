...
이렇게 보면 `EventSource`는 `WebSocket`보다 서버와 통신하는 능력이 부족한 수단입니다.

그런데도 `EventSource`를 써야 할 이유가 있을까요?

물론 있습니다. 단순하다는 점이죠. 상당수 애플리케이션엔 `WebSocket`이 제공하는 능력까지는 필요하지 않습니다.

채팅 메시지나 시세처럼 서버에서 흘러오는 데이터 스트림을 받기만 하면 되는 상황이라면 `EventSource`가 제격입니다. `EventSource`는 `WebSocket`에선 직접 구현해야 하는 자동 재연결도 지원합니다. 게다가 새로운 프로토콜이 아니라 익숙한 HTTP를 그대로 사용합니다.

## 메시지 받기

메시지를 받으려면 `new EventSource(url)`를 만들기만 하면 됩니다.

그러면 브라우저는 `url`로 접속해 커넥션을 열어둔 채 이벤트를 기다립니다.

서버는 상태 코드 200과 헤더 `Content-Type: text/event-stream`으로 응답한 뒤 커넥션을 유지하면서 다음과 같은 특별한 형식으로 메시지를 써 내려갑니다.
...
- 메시지 본문은 `data:` 뒤에 씁니다. 콜론 뒤 공백은 생략할 수 있습니다.
- 메시지와 메시지는 두 번의 줄바꿈 `\n\n`으로 구분합니다.
- 줄바꿈 `\n`을 보내고 싶으면 바로 다음 줄에 `data:`를 한 번 더 씁니다(위 예시의 세 번째 메시지).

실무에선 복잡한 메시지를 보통 JSON으로 인코딩해 보냅니다. 줄바꿈이 문자열 안에서 `\n`으로 인코딩되기 때문에 `data:`를 여러 줄로 나눠 쓸 일도 없습니다.
...
원격 서버는 `Origin` 헤더를 받고, 통신을 이어가려면 `Access-Control-Allow-Origin` 헤더로 응답해야 합니다.
...
`new EventSource`로 객체를 만들면 서버에 접속하고, 커넥션이 끊어지면 다시 접속합니다.

재연결을 직접 신경 쓰지 않아도 되니 아주 편리하죠.

재연결 사이엔 짧은 지연이 있는데 기본값은 몇 초 정도입니다.

서버는 응답에 `retry:`를 실어 권장 지연 시간을 밀리초 단위로 설정할 수 있습니다.
...
응답의 `Content-Type`이 올바르지 않거나 HTTP 상태 코드가 301, 307, 200, 204가 아닐 때도 재연결은 일어나지 않습니다. 이때는 `"error"` 이벤트가 발생하고 브라우저는 재연결을 시도하지 않습니다.
